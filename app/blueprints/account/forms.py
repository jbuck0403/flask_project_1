...
from app.validators import (
    notEmpty,
    verifyUserNameRequirements,
    verifyPasswordRequirements,
    verifyPassword,
    DELETE_ACCOUNT_KEYWORD,
    verifyDifferentUserName,
...
class SignupForm(FlaskForm):
    userName = StringField(
        "User Name", validators=[notEmpty, verifyUserNameRequirements]
    )
...
class UpdateAccountUserNameForm(AccountForm):
    changeUserName = StringField(
        "New User Name",
        validators=[notEmpty, verifyDifferentUserName, verifyUserNameRequirements],
    )
...
class UpdateAccountPasswordForm(AccountForm):
    newPassword = PasswordField(
        "New Password", validators=[verifyPasswordRequirements, verifyDifferentPassword]
    )
    ...
    currentPassword = PasswordField(
        "Current Password", validators=[notEmpty, verifyPassword]
    )
...
class DeleteAccountForm(AccountForm):
    verifyIntentionToDelete = StringField(
        f"Type '{DELETE_ACCOUNT_KEYWORD}'", validators=[confirmDeleteAccount]
    )
    userName = StringField(
        "User Name", validators=[notEmpty, confirmAccountExists, confirmUserLoggedIn]
    )
    password = PasswordField("Password", validators=[notEmpty, verifyPassword])
